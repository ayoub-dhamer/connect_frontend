...
export class ProjectListComponent implements OnInit {
  ...
  applyFilter() {
    ...
    this.filteredProjects = this.projects.filter((u) =>
  ...
  updateModalPagination() {
    this.modalTotalPages = Math.ceil(
      this.modalItems.length / this.modalPageSize,
    );
  ...
  getParticipantLabels(project: any): string[] {
    return project.participants.map((p: any) => `${p.name} (${p.email})`);
  }

  getTaskLabels(project: any): string[] {
    return project.tasks.map((t: any) => `${t.title} - ${t.status}`);
  }
}
